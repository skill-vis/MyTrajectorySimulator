...
ただ，ひとつだけお許しいただきたいことがあります．Rapsodoの角度は
...
入力として，
        ...
        ２．「縦のリリース角度」＝初速度の上下角度 (deg) ：　例，vel_angle_vertical_deg=-1.47, # deg　ボールの上下速度方向角度（下向きが正なので注意）
        
        ３．「横のリリース角度」＝初速度の左右角度 (deg)：　例，vel_azimuth_deg=2.0, # deg　と急の方位角　（例：右ピッチャーがマウンドの右側から，ホームベースの内側の左方向に投げるときは正）
        
        ４．「回転数」＝角速度のノルム　(回転数，rpm)：　例，spin_rate_rpm=2152.3, #　回転数
        
        ５．「回転方向」＝　投手から見た回転軸の水平面になす角度（deg）：　例，spin_tilt_deg=-25.0, # deg　投手から見た回転軸の水平軸に対する角度（ユーザが角度に変換してほしい。ここでは0としています）
        ...
        ６．「ジャイロ角度」＝回転軸の方位（真上から見た）（deg）：例，spin_azimuth_deg=0.0, # deg
