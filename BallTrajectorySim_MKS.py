...
class PitchParameters:
    ...
    # v0_mps: float = 42.5  # 初速度 (m/s) = 95 mph
    v0_mps: float = 38.9  # 初速度 (m/s) = 86.992 mph
    # theta_deg: float = -3.0  # リリース角度 (deg)
    theta_deg: float = -1.5  # リリース角度 (deg)
    # phi_deg: float = 0.0  # リリース方向 (deg)
    phi_deg: float = 3.0  # リリース方向 (deg)
    # backspin_rpm: float = 2300.0  # バックスピン (rpm)
    backspin_rpm: float = 1600.0  # バックスピン (rpm)
    # sidespin_rpm: float = 0.0  # サイドスピン (rpm)
    sidespin_rpm: float = 1000.0  # サイドスピン (rpm)
    # wg_rpm: float = 0.0  # 回転軸方向のスピン (rpm)
    wg_rpm: float = 900.0  # 回転軸方向のスピン (rpm)
...
class EnvironmentParameters:
    ...
    pressure_inHg: float = 29.92  # 気圧 (in Hg)
...
class BallTrajectorySimulator2:
    ...
    def __init__(self, integration_method: IntegrationMethod = IntegrationMethod.RK4):
        """
        初期化
        ...
        self.circumference_m = 0.229  # 円周 (m) = 9.125 inches
        self.radius_m = self.circumference_m / (2 * math.pi)  # 半径 (m)
        
        # 空気抵抗・揚力パラメータ
        self.cd0 = 0.3008  # 基本抗力係数
    ...
    def calculate_const(self, rho: float) -> float:
        ...
        """
        # SI単位系での定数計算
        # 元の式: 0.07182 * rho * (5.125 / mass_oz) * (circumference / 9.125)^2
        # 元の単位系: rho (lb/ft^3), mass (oz), circumference (inches)
        # 
        # 単位変換の詳細分析:
        # 元の式の単位: const = 0.07182 * (lb/ft^3) * (oz^-1) * (in^2)
        # SI単位系への変換:
        # - (lb/ft^3) → (kg/m^3): 1 lb/ft^3 = 16.0185 kg/m^3
        # - (oz^-1) → (kg^-1): 1 oz^-1 = 35.274 kg^-1
        # - (in^2) → (m^2): 1 in^2 = 0.00064516 m^2
        # 
        # 係数変換: 0.07182 * 16.0185 * 35.274 * 0.00064516 = 0.0261811905
        # 
        # 元の式の構造を保持し、参照値をSI単位系に変換
        const = 0.0261811905 * rho * (0.145 / self.mass_kg) * (self.circumference_m / 0.229) ** 2
        return const
    ...
    def calculate_drag_coefficient(self, v_rel: float, spin_eff: float, t: float) -> float:
        ...
        decay_term = 0 #math.exp(-t / (self.tau * v_ref_ms / v_rel)) if v_rel > 0 else 0
    ...
    def calculate_lift_coefficient(self, romega: float, v_rel: float, t: float) -> float:
        ...
        # 146.7 ft/s = 44.704 m/s (100 mph)
        v_ref_ms = 44.704  # 参照速度 (m/s)
        Q = (romega / v_rel) * math.exp(-t / (self.tau * v_ref_ms / v_rel)) if v_rel > 0 else 0
        
        # 揚力係数を計算
        cl = self.cl2 * Q / (self.cl0 + self.cl1 * Q) if (self.cl0 + self.cl1 * Q) > 0 else 0
        return cl
    ...
    def calculate_acceleration(self, state: np.ndarray, t: float, 
        ...
        spin_eff = math.sqrt(spin_total**2 - 
                            flag * ((30/math.pi) * (wx*vx + wy*vy + wz*vz) / v)**2) if v > 0 else spin_total
        
        # 回転速度（romega）
        romega = (spin_eff * math.pi / 30) * self.radius_m
    ...
    def simulate(self, pitch: Optional[PitchParameters] = None,
        ...
        omega_total = math.sqrt(pitch.backspin_rpm**2 + pitch.sidespin_rpm**2) * math.pi / 30 + 0.001
        ...
        wx = (-pitch.backspin_rpm * math.cos(math.radians(pitch.phi_deg)) - 
              ...
              pitch.wg_rpm * v0x / v0) * math.pi / 30
        wy = (pitch.backspin_rpm * math.sin(math.radians(pitch.phi_deg)) - 
              ...
              pitch.wg_rpm * v0y / v0) * math.pi / 30
        wz = (pitch.sidespin_rpm * math.cos(math.radians(pitch.theta_deg)) + 
              pitch.wg_rpm * v0z / v0) * math.pi / 30
        
        # 空気密度と定数を計算
        temp_C = (5/9) * (env.temp_F - 32)
        pressure_mmHg = env.pressure_inHg * 1000 / 39.37
        rho = self.calculate_air_density(temp_C, env.elev_m, env.relative_humidity, pressure_mmHg)
            ...
            if len(self.trajectory) > 0:
                ...
                if prev_y > home_plate_y and y <= home_plate_y and self.home_plate_crossing is None:
                    ...
                    spin_eff_home = math.sqrt(spin_total**2 - 
                                            flag * ((30/math.pi) * (wx*vx_home + wy*vy_home + wz*vz_home) / v_home)**2) if v_home > 0 else spin_total
                    romega_home = (spin_eff_home * math.pi / 30) * self.radius_m
    ...
    def get_summary(self):
        ...
        summary = {
            'initial_velocity_mph': initial['v_mph'],
            'final_velocity_mph': final['v_mph'],
...
def main():
    ...
    if summary:
        print("\n=== シミュレーション結果 ===")
        print(f"初速度: {summary['initial_velocity_mph']:.2f} mph")
        print(f"最終速度: {summary['final_velocity_mph']:.2f} mph")
        print(f"最大高度: {summary['max_height']:.2f} m")
        print(f"総時間: {summary['total_time']:.3f} sec")
        if summary['home_plate_crossing']:
            print(f"ホームプレート通過時速度: {summary['home_plate_crossing']['v_mph']:.2f} mph")
